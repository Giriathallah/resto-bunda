...
export default function CheckoutSuccessPage() {
  ...
  const mid = params.get("mid") || ""; // Midtrans order_id bila cashless
  ...
  useEffect(() => {
    fetchOrder();
    // tidak ada interval/auto-refresh
  ...
  async function manualCheckStatus() {
    ...
    setChecking(true);
    try {
      // Alur CASH: tidak punya 'mid' → tidak bisa cek ke Midtrans
      if (!isCashless) {
        await fetchOrder();
        alert(
          "Ini pesanan dengan pembayaran tunai. Mohon minta kasir menyelesaikan pembayaran, lalu klik refresh untuk memperbarui status."
        );
        return;
      }

      // Alur CASHLESS: cek status ke Midtrans menggunakan 'mid'
      ...
      // settlement atau capture+accept = sudah dibayar
      const isPaid =
      ...
      if (!isPaid) {
        alert(
          `Status pembayaran saat ini: ${s?.transaction_status || "unknown"}`
        );
        return;
      }
  ...
  if (loading) {
    return <div className="max-w-xl mx-auto px-4 py-8">Memuat…</div>;
  }
      ...
      <div className="bg-card border rounded-2xl p-6">
        ...
        {data.status !== "PAID" && (
          <div className="mt-6 flex flex-col gap-3 text-sm text-muted-foreground">
            <p>
              {isCashless
                ? "Setelah menyelesaikan pembayaran cashless, klik tombol di bawah untuk memperbarui status."
